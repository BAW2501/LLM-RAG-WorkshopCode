...
def get_word_to_vec_model(model_name="glove-wiki-gigaword-50"):
    filename = f"src/assets/{model_name}"
...
def get_direction_lines(
    axes, direction, n_lines=500, color=YELLOW, line_length=1.0, stroke_width=3
):
...
class LyingAboutTokens2(InteractiveScene):
    def construct(self):
        ...
        self.play(
            FadeOut(big_rect),
            LaggedStart(
                *(DrawBorderThenFill(rect) for rect in rects[:-1]), lag_ratio=0.02
            ),
            LaggedStart(
                *(word.animate.match_color(rect) for word, rect in zip(words, rects))
            ),
            FadeOut(arrow),
        )
        self.wait()
        ...
        token_rects = get_piece_rectangles(
            tokens, hue_range=(0.1, 0.2), leading_spaces=True, h_buff=0.0
        )
...
class ImageTokens(InteractiveScene):
    ...
    def construct(self):
        ...
        big_pixels = create_pixels(
            image, pixel_width=image.get_width() / (self.n_divisions / 4)
        )
        ...
        self.play(
            LaggedStart(
                (
                    patch.animate.set_stroke(TEAL, 3).set_anim_args(
...
class SoundTokens(InteractiveScene):
    def construct(self):
        ...
        self.play(
            LaggedStart(
                (
                    chunk.animate.set_stroke(TEAL, 3)
                    .scale(1.5)
...
class IntroduceEmbeddingMatrix(InteractiveScene):
    def construct(self):
        ...
        self.play(
            LaggedStartMap(
                FadeIn, shown_words, shift=0.5 * LEFT, lag_ratio=0.1, run_time=2
            ),
        ...
        column_rects = VGroup(
            *(SurroundingRectangle(column, buff=0.05) for column in columns)
        )
        ...
        self.play(
            frame.animate.set_width(FRAME_WIDTH + 3, about_edge=RIGHT), Write(lhs)
        )
        self.wait()
        ...
        for x in range(1):
            self.play(
                ...
                LaggedStartMap(
                    VShowPassingFlash, rects, lag_ratio=0.01, time_width=1.5
                ),
                run_time=2,
            )
        ...
        self.play(
            FadeOut(word_group, DOWN), matrix_group.animate.scale(2.0).move_to(frame)
        )
...
class Word2VecScene(InteractiveScene):
    ...
    def get_labeled_vector(
        ...
        if word in ['man','woman','king','queen']:
...
class AmbientWordEmbedding(Word2VecScene):
    def construct(self):
        ...
        # Add words
        words = "Coding Is The Best Club".split(" ")
        ...
        self.play(
            LaggedStartMap(
                FadeIn, pre_labels, shift=0.2 * UP, lag_ratio=0.1, run_time=1
            )
        )
...
class ThreeDSpaceExample(InteractiveScene):
    def construct(self):
        ...
        self.add(coords)
        for entry, line, label, value in zip(
            coords.get_entries(), lines, labels, coordinates
        ):
        ...
        def project(points):
            return np.array(
                [sum(np.dot(point, b) * b for b in bases) for point in points]
            )
...
class LearningEmbeddings(Word2VecScene):
    def construct(self):
        ...
        initial_coords = [
            rotate_vector(amp * OUT, theta, axis=UP) for theta, amp in zip(thetas, amps)
        ]
...
class KingQueenExample(Word2VecScene):
    ...
    def construct(self):
        ...
        man, woman, king, queen = word_vects = [
            ...
            for word, color, direction, coords in zip(
                words, colors, directions, all_coords
            )
        ]
        woman.label.shift(SMALL_BUFF * DOWN)

        fake_queen_coords = (
            all_coords[2] - all_coords[0] + all_coords[1]
        )  # Tweak queen for demo purposes
        ...
        self.play(
            ...
            frame.animate.reorient(0, 0, 0, (2.04, 2.06, 0.38), 4.76).set_anim_args(
                run_time=6
            ),
        )
        self.play(
        ...
        self.play(
            ...
            *map(Write, [eq, minus1, ek, approx]),
            ...
            frame.animate.reorient(0, 2, 0, (0.04, 1.96, -0.13), 5.51).set_anim_args(
                run_time=3
            ),
        )
        self.play(
            frame.animate.reorient(-110, 10, 110, (0.22, 1.6, -0.07), 6.72), run_time=10
        )
        ...
        n_circs = 5
        src_circles = (
            Circle(radius=1e-2).set_stroke(width=5, opacity=1).replicate(n_circs)
        )
        ...
        turn_animation_into_updater(
            ApplyMethod(
                frame.reorient, -116, 21, 114, (0.37, 1.45, 0.23), 7.59, run_time=12
            )
        )
        ...
        for word1, word2 in word_pairs:
            ...
            vect1 = self.get_labeled_vector(
                word1, color=colors[2], label_config=label_config, coords=new_coords[0]
            )
            vect2 = self.get_labeled_vector(
                word2, color=colors[3], label_config=label_config, coords=new_coords[1]
            )
        ...
        self.play(
            LaggedStartMap(
                VShowPassingFlash, lines, lag_ratio=1 / len(lines), run_time=4
            )
        )
...
class HitlerMussoliniExample(KingQueenExample):
    ...
    def construct(self):
        ...
        self.play(
            TransformFromCopy(diff, diff_copy), rect.animate.surround(equation[:5])
        )
        self.wait(2)
        self.play(
    ...
    def construct(self):
        ...
        for vect_group in vect_groups:
            ...
            for vect in vect_group:
                ...
                vect.label.next_to(
                    vect.get_end(), normalize(vect.get_vector()), SMALL_BUFF
                )
...
class RicherEmbedding(InteractiveScene):
    def construct(self):
        ...
        self.play(
            ...
            frame.animate.reorient(2, -16, 0, (0.6, -0.04, 0.02), 6.01).set_anim_args(
                run_time=8
            ),
        )
        self.play(
            ...
            frame.animate.reorient(35, -23, 0, (0.6, -0.04, 0.02), 6.01).set_anim_args(
                run_time=5
            ),
        )
        self.play(
            ...
            frame.animate.reorient(20, -29, 0, (0.61, 0.01, 0.0), 6.10).set_anim_args(
                run_time=5
            ),
        )
        self.play(
            frame.animate.reorient(-19, -25, 0, (0.61, 0.01, 0.0), 6.10), run_time=5
        )
...
class RefineTowerMeaning(MultipleMoleEmbeddings):
    def construct(self):
        ...
        eiffel_tower_images = Group(
            ImageMobject(f"EiffelTower{n}") for n in range(1, 4)
        )
        mini_eiffel_tower_images = Group(ImageMobject("MiniEiffelTower1"))
        image_groups = Group(
            tower_images, eiffel_tower_images, mini_eiffel_tower_images
        )
        ...
        self.play(
            ...
            eiffel_tower.animate.set_opacity(1)
        ...
        self.play(
            frame.animate.reorient(-14, -2, 0, (-0.12, 2.21, 0.72), 7.05).set_anim_args(
                run_time=2
            ),
...
class UpdatingPoetryEmbedding(RicherEmbedding):
    def construct(self):
        ...
        self.play(
            frame.animate.reorient(22, -23, 0, (-0.86, 0.4, -0.35), 7.15), run_time=5
        )
...
class ManyIdeasManyDirections(SimpleSpaceExample):
    ...
    def construct(self):
        ...
        for idea in ideas:
            ...
            lines = get_direction_lines(
                axes, direction, color=new_vect.get_color(), n_lines=250, stroke_width=2
            )
...
class MJSpace(SimpleSpaceExample):
    def construct(self):
        ...
        arrows = VGroup(
            Arrow(rect, arr, buff=0.1) for rect, arr in zip(token_rects, arrs)
        )
        ...
        labels = VGroup(
            *map(Text, ["First Name Michael", "Last Name Jordan", "Basketball"])
        )
        label_directions = [LEFT + OUT, IN, RIGHT + OUT]

        vect_groups = VGroup()
        vects = VGroup()
        for coords, label, color, direction in zip(
            all_coords, labels, colors, label_directions
        ):
        ...
        def get_dot_product_lines(vect, proj_line_color=GREY_A):
            dashed_line = always_redraw(
                lambda: Line(emb.get_end(), get_proj_point(emb, vect))
                .set_stroke(WHITE, 2)
                .set_anti_alias_width(10)
            )
            proj_line = always_redraw(
                lambda: Line(ORIGIN, get_proj_point(emb, vect)).set_stroke(
                    proj_line_color, width=4, opacity=0.75
                )
            )
        ...
        rhs.add_updater(
            lambda m: m.set_value(np.dot(m.target_vect.get_end(), emb.get_end()) / 4.0)
        )
        ...
        self.play(
            ...
            TransformFromCopy(
                Line(ORIGIN, emb.get_end(), flat_stroke=False), m_proj_line
            ),
            FadeIn(formula, UP),
        ...
        self.play(
            TransformFromCopy(
                rhs.copy().unfix_from_frame().set_opacity(0).move_to(m_proj_line), rhs
            ),
        )
        ...
        self.wait(2)
        self.play(
            emb.animate.put_start_and_end_on(
                axes.get_origin(), 1.5 * all_coords[1:3].sum(0)
            ),
            run_time=3,
        )
        self.play(frame.animate.reorient(26, 68, 0), run_time=2)
        self.play(
            emb.animate.put_start_and_end_on(ORIGIN, [1.0, -1.5, -1.0]), run_time=3
        )
        self.wait(2)
        self.play(
        ...
        j_paren = Tex(
            R"\big(\overrightarrow{\text{Last Name Jordan}}\big) = ", font_size=36
        )
        ...
        self.play(
            ...
            rhs.animate.next_to(j_paren, RIGHT, buff=0.15).set_anim_args(
                time_span=(1, 2)
            ),
            LaggedStart(
        ...
        self.play(
            ...
            TransformFromCopy(
                Line(ORIGIN, emb.get_end(), flat_stroke=False), j_proj_line
            ),
        )
